...
(defun my-configure-fonts ()
  ...
  (let ((base-height font-height)
        ...
        (symbol-font-family (my-find-available-font '("JetBrainsMono NF" "Symbola" "Noto Sans Symbols" "Monospace")))
    ...
    (when (fboundp 'set-fontset-font)
      ...
      (set-fontset-font t #x27FC "Segoe UI Symbol" nil 'prepend))))         ;; (long right arrow from bar)
...
(use-package pretty-mode
  ...
  (pretty-deactivate-patterns
   '(:== :neq :leq :geq :wedge :vee :dots :circ :===))
...
(unless (is-android)
  ...
  (use-package tidal
    :after haskell-mode
    :config
    (define-key tidal-mode-map (kbd "C-c C-.") 'tidal-run-line))
  (use-package tidal-highlight
    ...
    :ensure (tidal-highlight
	     ...
	     :files ("*.el" "bin"))
    :config
    (setq tidal-highlight-udp-port 6012)))

(custom-set-faces
 '(pulse-highlight-face ((t (:background "#2aa198" :foreground "#c6c6c6"))))
 '(pulse-highlight-start-face ((t (:background "#89b4fa" :foreground "#000000")))))
#+end_src
** Treemacs
#+begin_src emacs-lisp :tangle ~/.emacs.d/config.el
...
(when (executable-find "deno")
  ...
  (use-package emmet2-mode
    ...
    :after deno-bridge
    :hook ((web-mode css-mode) . emmet2-mode)
    :config
...
(defun my-def-rep-command (alist &optional initial-key)
  "Create a command that runs the function for INITIAL-KEY once, then
sets a transient keymap for repeated actions from ALIST."
  (let* ((initial-key (or initial-key (caar alist)))
    ...
    `(lambda (arg)
       (interactive "P")
       (when ',initial-func
         (call-interactively ',initial-func (list arg)))
...
(defun org-split-src-block ()
  "Split the current source block at point into two source blocks."
  (interactive)
  (save-excursion
    (unless (org-in-src-block-p)
      (error "Not inside a source block"))

    (let ((current-line (string-trim (buffer-substring (point-at-bol) (point-at-eol))))
          (language (org-element-property :language (org-element-context)))
          (headers (org-element-property :parameters (org-element-context))))
      (beginning-of-line)
      (kill-whole-line)
      (insert "#+end_src\n")
      (insert "\n")
...
(defun org-merge-src-blocks ()
  "Merge the source block above point with the source block below point."
  (interactive)
  (save-excursion
    (beginning-of-line)
    (let ((has-block-above (save-excursion
                             (re-search-backward "^#\\+end_src" nil t)))
          (has-block-below (save-excursion
                             (re-search-forward "^#\\+begin_src" nil t))))

      (unless (and has-block-above has-block-below)
        (error "Not positioned between two source blocks"))
...
(defun move-region (start end n)
  ...
  (let* (
         ...
         (line-text (delete-and-extract-region start-pos end-pos)))
    ...
    (let ((new-start (point)))
      ...
      (set-mark new-start)
      (goto-char (+ new-start (length line-text))))))
...
(defun my-move-smart-up (n)
  "Move the active region (if any) or the current line up by N lines."
  (interactive "p")
  (if (use-region-p)
      (move-region-up (region-beginning) (region-end) n)
    (move-line-up n)))

(defun my-move-smart-down (n)
  "Move the active region (if any) or the current line down by N lines."
  (interactive "p")
  (if (use-region-p)
...
(defvar my-line-move-repeat-cmds
  '(("<up>" . my-move-smart-up)
    ("<down>" . my-move-smart-down)
    ("q" . (lambda () (interactive) (message "Exited line movement repetition."))))
  "Commands for repeatable line/region movement.")
...
(use-package general
  ...
  (leader-key
    ...
    "r <up>" `(,(my-def-rep-command my-line-move-repeat-cmds "<up>")
               :wk "Move line/region up (Up/Down to repeat, q to quit)")
    "r <down>" `(,(my-def-rep-command my-line-move-repeat-cmds "<down>")
		 :wk "Move line/region down (Up/Down to repeat, q to quit)"))
  (leader-key
    "e" '(:ignore t :wk "evals")
  ...
  (leader-key
    ...
    "s m" '(sp-mark-sexp :wk "Mark surrounding sexp")
    ...
    "s B" '(sp-beginning-of-sexp :wk "Beginning of sexp")
    ...
    "s s <delete>" '(sp-unwrap-sexp :wk "Unwrap sexp")
    "s s <backspace>" '(sp-backward-unwrap-sexp :wk "Backward unwrap sexp")
    ...
    "s w" '(:ignore t :wk "wrap")
    "s w (" '(sp-wrap-round :wk "Wrap in parentheses ()")
    "s w [" '(sp-wrap-square :wk "Wrap in square brackets []")
    "s w {" '(sp-wrap-curly :wk "Wrap in curly braces {}")
    "s M-d" '(sp-splice-sexp :wk "Splice sexp")
    "s C-<delete>" '(sp-splice-sexp-killing-forward :wk "Splice and kill forward")
    "s C-<backspace>" '(sp-splice-sexp-killing-backward :wk "Splice and kill backward")
    "s C-S-<backspace>" '(sp-splice-sexp-killing-around :wk "Splice and kill around")
