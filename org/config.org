...
(unless (boundp 'text-scale-mode-step)
  (defvar text-scale-mode-step 1.2
    "Scaling factor used by text-scale-mode."))
...
(defvar my-original-face-heights nil
  "A list of cons cells (face . height) storing original face heights.")

(defun my-capture-original-face-heights ()
  "Capture the original height of faces we intend to scale."
  (unless my-original-face-heights
    (dolist (face '(default
                    ...
                    vertico-multiline))
      (push (cons face font-height) my-original-face-heights))))
...
(defun my-text-scale-reset-all-faces ()
  ...
  (when (/= my-font-scaling-steps 0)
    ...
    (my-scale-all-managed-faces 1.0)))

(my-capture-original-face-heights)

(add-hook 'server-after-make-frame-hook (lambda (&rest _) (my-capture-original-face-heights)))
(add-hook 'elpaca-after-init-hook (lambda (&rest _) (my-capture-original-face-heights)))
#+end_src
* Packages
** Completion
#+begin_src emacs-lisp :tangle ~/.emacs.d/config.el
